...
uint8_t buffer_Rx_MCU[BUFFER_SIZE];
uint8_t buffer_Rx_aaa[BUFFER_SIZE];
uint8_t size_rx_MCU;
  ...
  while (1)
  {
    ...
    //MX_SubGHz_Phy_Process();
	  ...
	  if(flag_mcu_send_node){
		  flag_mcu_send_node = false;
		  Radio.Send(buffer_Rx_MCU, size_rx_MCU);
		  Radio.Rx(0);
	  }else if (g_lora_data_received){
		  g_lora_data_received= false;
